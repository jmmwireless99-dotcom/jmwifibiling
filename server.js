...
import { routerConfigFromRecord, createMtClient, testRouterConnection } from "./lib/routers.js";
...
import { verifyWebhook as kitifiVerify, parsePaymentEvent as kitifiParse, isPaidStatus as kitifiPaid, profileForAmount as kitifiProfileForAmount } from "./lib/kitifi.js";
import { Collections, Audit, Sales } from "./lib/db.js";
import { HotspotEvents, Coins, HotspotSales, KitifiOrders } from "./lib/db.js";
...
import { kpis, Naps, Olts, FiberPatches, Outages, Reports, CSessions, ClientStatus, Sms } from "./lib/db.js";
...
let mt;
function mtConfig(routerId) {
  let s = {}; try { s = Settings.all(); } catch {}
  let pr = null;
  try {
    pr = routerId ? Routers.get(Number(routerId)) : Routers.primary();
  } catch {}
  if (pr && pr.host && pr.enabled !== 0) return routerConfigFromRecord(pr);
...
function rebuildMt() {
  const c = mtConfig();
  if (MODE === "rest") {
    mt = new MikroTik({ host: c.host, user: c.user, password: c.password, verifyTls: process.env.MIKROTIK_TLS_VERIFY === "true" });
    return;
  }
  mt = createMtClient(c);
}
...
const GET_ROUTES = {
  ...
  "/api/router/test": async (req) => {
    const q = new URL(req.url || "", "http://localhost").searchParams;
    const rid = q.get("router_id");
    const c = mtConfig(rid || undefined);
    if (!c.host) return { connected: false, error: "No MikroTik router configured. Add one under MikroTik Routers." };
    const r = await testRouterConnection(c);
    if (rid && r.connected) try { Routers.setStatus(Number(rid), "online"); } catch {}
    if (rid && !r.connected) try { Routers.setStatus(Number(rid), r.error || "offline"); } catch {}
    return r;
  },
...
const requestHandler = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/manifest.webmanifest") {
      const biz = Settings.get("biz_name", "JM Billing System");
    ...
    if (pathname === "/api/billing/paymongo/webhook" && req.method === "POST") {
      ...
      if (ev && /paid$/.test(ev.type || "")) {
        ...
        if (inv && inv.status !== "paid") {
          if (ev.amount && Math.abs(Number(ev.amount) - Math.round(Number(inv.amount) * 100)) > 1) {
            Audit.add({ type: "auto", customer_id: inv.customer_id, customer_name: inv.customer_name, action: "paymongo-amount-mismatch", detail: `expected ${Math.round(Number(inv.amount) * 100)} got ${ev.amount}`, ok: false });
          }
          ...
          try {
            ...
            if (cust) {
              const newExpiry = await renewCustomer(cust, cust.plan_mins || 43200);
              Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "renew-reconnect", detail: "PayMongo · expiry " + newExpiry, ok: true });
              tgNotify(`✅ <b>ONLINE PAYMENT</b>\nUser: <b>${escapeHtml(cust.username || cust.name)}</b>\nInvoice #${inv.id} · PayMongo\nNew expiry: <b>${newExpiry}</b>`);
              if (Settings.get("notify_customers", "1") === "1" && cust.contact) {
                notifyCustomerMsg(cust.contact, cust.name, "Payment received — reconnected", `Hi ${cust.name}, we received your online payment and your internet is reconnected. New expiry: ${newExpiry}. Thank you!`).catch(() => {});
              }
              maybeAutoReceipt(cust.id).catch(() => {});
            }
          } catch (e) {
            Audit.add({ type: "auto", customer_id: inv.customer_id, action: "paymongo-renew-failed", detail: e.message, ok: false });
          }
        } else if (!inv) {
          Audit.add({ type: "auto", action: "paymongo-no-invoice", detail: (ev.resourceId || "") + " " + (ev.remarks || ""), ok: false });
        }
      }
      res.writeHead(200, { "Content-Type": "text/plain" });
      return res.end("OK");
    ...
    // KitFi Pisowifi GCash webhook — creates hotspot voucher on paid event
    if (pathname === "/api/billing/kitifi/webhook" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      const secret = Settings.get("kitifi_webhook_secret") || Settings.get("kitifi_api_key");
      if (secret && !kitifiVerify(req, raw, secret)) { res.writeHead(401); return res.end("unauthorized"); }
      let json = null;
      try { json = JSON.parse(raw); } catch {}
      const ev = kitifiParse(json);
      if (!ev) { res.writeHead(200); return res.end("OK"); }
      const ref = ev.reference || ("kitifi-" + Date.now());
      const existing = KitifiOrders.byReference(ref);
      if (existing && existing.status === "paid" && existing.voucher) {
        res.writeHead(200); return res.end("OK");
      }
      if (kitifiPaid(ev.status) || (!ev.status && ev.amount > 0 && ev.voucher)) {
        try {
          const profile = ev.profile
            || kitifiProfileForAmount(ev.amount, Settings.get("kitifi_plan_map"), Settings.get("kitifi_default_profile", "default"));
          const uptime = ev.uptime || Settings.get("kitifi_default_uptime", "");
          let voucher = ev.voucher;
          if (!voucher) {
            voucher = genVoucherCode(Number(Settings.get("kitifi_code_length", "6")) || 6, Settings.get("kitifi_code_prefix", ""));
            await mt.createHotspotUser({ name: voucher, password: "", profile, limitUptime: uptime });
          } else {
            try { await mt.createHotspotUser({ name: voucher, password: "", profile, limitUptime: uptime }); } catch {}
          }
          const order = KitifiOrders.upsert({
            reference: ref, amount: ev.amount, status: "paid", voucher, profile, phone: ev.phone, mac: ev.mac,
            paid_at: new Date().toISOString(), raw_payload: raw.slice(0, 4000),
          });
          HotspotEvents.add({ type: "kitifi", user: voucher, amount: ev.amount, mac: ev.mac, detail: `GCash ₱${ev.amount} · ${profile}`, vendo: "KitFi", device: ev.phone || "" });
          Audit.add({ type: "auto", action: "kitifi-voucher", detail: `${voucher} ₱${ev.amount} ${profile}`, ok: true });
          tgNotify(`🎟 <b>KITFI GCASH</b>\nVoucher: <b>${escapeHtml(voucher)}</b>\nAmount: ₱${ev.amount}\nProfile: ${escapeHtml(profile)}${ev.phone ? "\nPhone: " + escapeHtml(ev.phone) : ""}`);
          if (Settings.get("notify_customers", "1") === "1" && ev.phone && looksLikePhone(ev.phone)) {
            notifyCustomerMsg(ev.phone, voucher, "Your WiFi voucher", `Your voucher code is ${voucher}. Amount paid: ₱${ev.amount}. Thank you!`).catch(() => {});
          }
        } catch (e) {
          KitifiOrders.upsert({ reference: ref, amount: ev.amount, status: "error", phone: ev.phone, mac: ev.mac, raw_payload: raw.slice(0, 4000) });
          Audit.add({ type: "auto", action: "kitifi-voucher-failed", detail: e.message, ok: false });
        }
      } else {
        KitifiOrders.upsert({ reference: ref, amount: ev.amount, status: ev.status || "pending", phone: ev.phone, mac: ev.mac, raw_payload: raw.slice(0, 4000) });
      }
      res.writeHead(200, { "Content-Type": "text/plain" });
      return res.end("OK");
    }
    ...
    if (req.method === "GET" && GET_ROUTES[pathname]) {
      try {
        return send(res, 200, { ok: true, data: await GET_ROUTES[pathname](req) });
      } catch (e) {
        return send(res, 500, { ok: false, error: e.message });
      }
    }

    // ---- MikroTik routers (multi-router registry) ----
    if (pathname.startsWith("/api/routers")) {
      if (!need()) return denied();
      const subRaw = pathname.slice("/api/routers".length);
      const sub = subRaw === "" || subRaw === "/" ? "" : subRaw;
      const body = ["POST", "PATCH", "PUT"].includes(req.method)
        ? JSON.parse((await readBody(req)) || "{}") : {};
      let mm;
      if (sub === "" && req.method === "GET") {
        const rows = Routers.list().map(Routers.public);
        const primary = Routers.primary();
        return send(res, 200, { ok: true, data: rows, primaryId: primary ? primary.id : null });
      }
      if (sub === "" && req.method === "POST") {
        try {
          const r = Routers.create(body);
          if (r.is_primary) { try { mt.close && mt.close(); } catch {} rebuildMt(); }
          Audit.add({ type: "manual", action: "router-add", detail: r.name + " " + r.host, ok: true });
          return send(res, 200, { ok: true, data: Routers.public(r) });
        } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
      }
      if ((mm = sub.match(/^\/(\d+)\/test$/)) && req.method === "POST") {
        const row = Routers.get(Number(mm[1]));
        if (!row) return send(res, 404, { ok: false, error: "Router not found" });
        const cfg = routerConfigFromRecord(row);
        const r = await testRouterConnection(cfg);
        Routers.setStatus(row.id, r.connected ? "online" : (r.error || "offline"));
        return send(res, 200, { ok: true, data: r });
      }
      if ((mm = sub.match(/^\/(\d+)\/primary$/)) && req.method === "POST") {
        try {
          const r = Routers.setPrimary(Number(mm[1]));
          try { mt.close && mt.close(); } catch {} rebuildMt();
          Audit.add({ type: "manual", action: "router-primary", detail: "#" + r.id + " " + r.name, ok: true });
          return send(res, 200, { ok: true, data: Routers.public(r) });
        } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
      }
      if ((mm = sub.match(/^\/(\d+)$/)) && (req.method === "PATCH" || req.method === "PUT")) {
        try {
          const r = Routers.update(Number(mm[1]), body);
          if (r.is_primary) { try { mt.close && mt.close(); } catch {} rebuildMt(); }
          Audit.add({ type: "manual", action: "router-edit", detail: "#" + r.id + " " + r.name, ok: true });
          return send(res, 200, { ok: true, data: Routers.public(r) });
        } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
      }
      if ((mm = sub.match(/^\/(\d+)$/)) && req.method === "DELETE") {
        try {
          const row = Routers.get(Number(mm[1]));
          Routers.remove(Number(mm[1]));
          try { mt.close && mt.close(); } catch {} rebuildMt();
          Audit.add({ type: "manual", action: "router-delete", detail: row ? row.name : "#" + mm[1], ok: true });
  ...
  <script>
    ...
    function getLoc(){
      ...
      navigator.geolocation.getCurrentPosition(function(pos){
        ...
        $("f-pin").textContent="📍 Location pinned (coordinates saved below)"; $("f-pin").classList.add("set");
  ...
  try {
    ...
    if (sub === "/map" && method === "GET") {
      return ok({
        ...
        naps: Naps.list(),
        olts: Olts.list(),
        patches: FiberPatches.list().map(FiberPatches.public),
      });
    }
    if (sub === "/naps" && method === "GET") return ok(Naps.list());
    if (sub === "/olts" && method === "GET") return ok(Olts.list());
    if (sub === "/patches" && method === "GET") return ok(FiberPatches.list().map(FiberPatches.public));
    ...
    if (sub === "/patches" && method === "POST") {
      try {
        const p = FiberPatches.create(body);
        Audit.add({ type: "manual", action: "patch-add", detail: "patch #" + p.id, ok: true });
        return ok(FiberPatches.public(p));
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/patches\/(\d+)$/)) && (method === "PATCH" || method === "PUT")) {
      try {
        const p = FiberPatches.update(Number(mm[1]), body);
        Audit.add({ type: "manual", action: "patch-edit", detail: "#" + mm[1], ok: true });
        return ok(FiberPatches.public(p));
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/patches\/(\d+)$/)) && method === "DELETE") {
      FiberPatches.remove(Number(mm[1]));
      Audit.add({ type: "manual", action: "patch-delete", detail: "#" + mm[1], ok: true });
      return ok({ deleted: true });
    }
    ...
    if (sub === "/kitifi/orders" && method === "GET") {
      return ok(KitifiOrders.recent(80));
    }

    // ---- Business / SMTP settings (admin only) ----
    if (sub === "/settings" && method === "GET") {
      const s = Settings.all();
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key", "portal_token", "kitifi_api_key", "kitifi_webhook_secret"]) if (s[k]) s[k] = "***";
      return ok(s);
    }
    if (sub === "/settings" && method === "POST") {
      const patch = { ...body };
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key", "portal_token", "kitifi_api_key", "kitifi_webhook_secret"])
        if (patch[k] === "***" || patch[k] === undefined) delete patch[k]; // keep existing
      Settings.setMany(patch);
      if (Object.keys(patch).some((k) => k.startsWith("mikrotik_"))) {
        try {
          const pr = Routers.primary();
          if (pr) {
            Routers.update(pr.id, {
              host: patch.mikrotik_host ?? pr.host,
              port: patch.mikrotik_port != null ? patch.mikrotik_port : pr.port,
              username: patch.mikrotik_user ?? pr.username,
              password: patch.mikrotik_password,
              ssl: patch.mikrotik_ssl != null ? patch.mikrotik_ssl === "1" : !!pr.ssl,
            });
          } else if (patch.mikrotik_host) {
            Routers.create({
              name: Settings.get("biz_name", "") ? Settings.get("biz_name", "") + " Router" : "Primary Router",
              host: patch.mikrotik_host,
              port: patch.mikrotik_port,
              username: patch.mikrotik_user,
              password: patch.mikrotik_password,
              ssl: patch.mikrotik_ssl === "1",
              is_primary: true,
            });
          }
        } catch {}
...
server.listen(PORT, () => {
  ...
  if (!Settings.get("biz_name")) Settings.set("biz_name", "JM Billing System");
  ...
  console.log("============================================");
  console.log(" JM Billing System Panel");
  console.log(" Open in your browser: " + url);
  console.log(" Router target: " + (mtConfig().host || "(add a router under MikroTik Routers)") + "  [mode: " + MODE + (MODE === "rest" ? "" : " port " + (mt.port || mtConfig().port)) + "]");
