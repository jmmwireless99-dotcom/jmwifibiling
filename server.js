...
const requestHandler = async (req, res) => {
  ...
  try {
    ...
    if (pathname === "/api/pay-start" && req.method === "POST") {
      ...
      const sk = Settings.get("paymongo_secret", "") || process.env.PAYMONGO_SECRET || "";
      if (!sk) return send(res, 503, { ok: false, error: "Online payment isn't set up yet. Please use GCash/Maya manual payment for now." });
      try {
        const s = Settings.all();
        const link = await pmCreateLink(
          { secret: sk, baseUrl: s.paymongo_base || undefined },
...
async function handleBilling(req, res, pathname) {
  ...
  try {
    ...
    if (sub === "/routers" && method === "GET") {
      // never leak passwords to the browser; show whether one is set
      return ok(Routers.list().map(({ password, ...r }) => ({ ...r, hasPassword: !!password })));
    }
    ...
    if ((mm = m(/^\/customers\/(\d+)\/router$/)) && method === "POST") {
      Customers.setRouter(Number(mm[1]), body.router_id);
      return ok(Customers.get(Number(mm[1])));
    }
