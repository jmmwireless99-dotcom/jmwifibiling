...
db.exec(`CREATE TABLE IF NOT EXISTS kitifi_orders (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
// KitFi GCash payment orders (webhook → auto voucher on MikroTik).
export const KitifiOrders = {
  byReference: (ref) => get("SELECT * FROM kitifi_orders WHERE reference=?", String(ref || "")),
  recent: (limit = 50) => all("SELECT * FROM kitifi_orders ORDER BY id DESC LIMIT ?", Number(limit) || 50),
  upsert: (o) => {
    const ref = String(o.reference || "").trim();
    if (!ref) {
      const r = run(
        "INSERT INTO kitifi_orders (reference,amount,status,voucher,profile,phone,mac,paid_at,raw_payload) VALUES (?,?,?,?,?,?,?,?,?)",
        "", Number(o.amount) || 0, o.status || "pending", o.voucher || "", o.profile || "", o.phone || "", o.mac || "",
        o.paid_at || null, o.raw_payload || ""
      );
      return get("SELECT * FROM kitifi_orders WHERE id=?", r.lastInsertRowid);
    }
    const existing = get("SELECT * FROM kitifi_orders WHERE reference=?", ref);
    if (existing) {
      run(
        "UPDATE kitifi_orders SET amount=?,status=?,voucher=?,profile=?,phone=?,mac=?,paid_at=COALESCE(?,paid_at),raw_payload=? WHERE id=?",
        Number(o.amount) || existing.amount || 0, o.status || existing.status, o.voucher || existing.voucher || "",
        o.profile || existing.profile || "", o.phone || existing.phone || "", o.mac || existing.mac || "",
        o.paid_at || null, o.raw_payload || existing.raw_payload || "", existing.id
      );
      return get("SELECT * FROM kitifi_orders WHERE id=?", existing.id);
    }
    const r = run(
      "INSERT INTO kitifi_orders (reference,amount,status,voucher,profile,phone,mac,paid_at,raw_payload) VALUES (?,?,?,?,?,?,?,?,?)",
      ref, Number(o.amount) || 0, o.status || "pending", o.voucher || "", o.profile || "", o.phone || "", o.mac || "",
      o.paid_at || null, o.raw_payload || ""
    );
    return get("SELECT * FROM kitifi_orders WHERE id=?", r.lastInsertRowid);
  },
...
const BACKUP_TABLES = ["settings", "plans", "customers", "invoices", "payments", "users", "vendos", "tickets", "payment_proofs", "mikrotik_routers", "naps", "olts", "fiber_patches"];
...
db.exec(`CREATE TABLE IF NOT EXISTS mikrotik_routers (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  host TEXT NOT NULL,
...
ensureColumn("customers", "router_id", "INTEGER");

(function migrateLegacyRouter() {
  try {
    const n = get("SELECT COUNT(*) c FROM mikrotik_routers").c;
    if (n > 0) return;
    const host = Settings.get("mikrotik_host", "");
    if (!host) return;
    run(
      "INSERT INTO mikrotik_routers (name,host,port,username,password,ssl,is_primary,enabled) VALUES (?,?,?,?,?,?,1,1)",
      Settings.get("biz_name", "") ? Settings.get("biz_name", "") + " Router" : "Primary Router",
      host.split(":")[0],
      Number(Settings.get("mikrotik_port", "8728")) || 8728,
      Settings.get("mikrotik_user", ""),
      Settings.get("mikrotik_password", ""),
      Settings.get("mikrotik_ssl", "0") === "1" ? 1 : 0
    );
  } catch {}
})();

export const Routers = {
  list: () => all("SELECT * FROM mikrotik_routers ORDER BY is_primary DESC, name COLLATE NOCASE"),
  get: (id) => get("SELECT * FROM mikrotik_routers WHERE id=?", id),
  primary: () => get("SELECT * FROM mikrotik_routers WHERE is_primary=1 AND enabled=1 LIMIT 1")
    || get("SELECT * FROM mikrotik_routers WHERE enabled=1 ORDER BY id LIMIT 1"),
  public: (r) => {
    if (!r) return null;
    return { ...r, password: r.password ? "***" : "" };
  },
  syncSettings: (r) => {
    if (!r) return;
    Settings.set("mikrotik_host", r.host || "");
    Settings.set("mikrotik_port", String(r.port || 8728));
    Settings.set("mikrotik_user", r.username || "");
    if (r.password) Settings.set("mikrotik_password", r.password);
    Settings.set("mikrotik_ssl", r.ssl ? "1" : "0");
  },
  create: (r) => {
    const row = {
      name: String(r.name || "").trim(),
      host: String(r.host || "").split(":")[0].trim(),
    ...
    if (!row.name || !row.host) throw new Error("Name and host are required.");
    const dup = get(
      "SELECT id FROM mikrotik_routers WHERE lower(host)=lower(?) AND port=?",
      row.host, row.port
    );
    if (dup) throw new Error("A router with this host and port already exists.");
    const count = get("SELECT COUNT(*) c FROM mikrotik_routers").c;
    const isPrimary = count === 0 || r.is_primary ? 1 : 0;
    if (isPrimary) run("UPDATE mikrotik_routers SET is_primary=0");
    const ins = run(
      "INSERT INTO mikrotik_routers (name,host,port,username,password,ssl,notes,enabled,is_primary) VALUES (?,?,?,?,?,?,?,?,?)",
      row.name, row.host, row.port, row.username, row.password, row.ssl, row.notes, row.enabled, isPrimary
    );
    const created = Routers.get(ins.lastInsertRowid);
    if (isPrimary) Routers.syncSettings(created);
    return created;
  },
  update: (id, r) => {
    const cur = Routers.get(id);
    if (!cur) throw new Error("Router not found");
    const row = {
      name: r.name != null ? String(r.name).trim() : cur.name,
      host: r.host != null ? String(r.host).split(":")[0].trim() : cur.host,
      port: r.port != null ? Number(r.port) || 8728 : cur.port,
      username: r.username != null ? r.username : cur.username,
      password: r.password != null && r.password !== "***" ? r.password : cur.password,
      ssl: r.ssl != null ? (r.ssl ? 1 : 0) : cur.ssl,
      notes: r.notes != null ? r.notes : cur.notes,
      enabled: r.enabled != null ? (r.enabled ? 1 : 0) : cur.enabled,
    };
    if (!row.name || !row.host) throw new Error("Name and host are required.");
    const dup = get(
      "SELECT id FROM mikrotik_routers WHERE lower(host)=lower(?) AND port=? AND id<>?",
      row.host, row.port, id
    );
    if (dup) throw new Error("A router with this host and port already exists.");
    run(
      "UPDATE mikrotik_routers SET name=?,host=?,port=?,username=?,password=?,ssl=?,notes=?,enabled=? WHERE id=?",
      row.name, row.host, row.port, row.username, row.password, row.ssl, row.notes, row.enabled, id
    );
    const updated = Routers.get(id);
    if (updated.is_primary) Routers.syncSettings(updated);
    return updated;
  },
  setPrimary: (id) => {
    const r = Routers.get(id);
    if (!r) throw new Error("Router not found");
    run("UPDATE mikrotik_routers SET is_primary=0");
    run("UPDATE mikrotik_routers SET is_primary=1, enabled=1 WHERE id=?", id);
    const updated = Routers.get(id);
    Routers.syncSettings(updated);
    return updated;
  },
  setStatus: (id, status) => {
    run("UPDATE mikrotik_routers SET last_status=?, last_checked=datetime('now') WHERE id=?", status || "", id);
    return Routers.get(id);
  },
  remove: (id) => {
    const r = Routers.get(id);
    if (!r) throw new Error("Router not found");
    const wasPrimary = !!r.is_primary;
    run("DELETE FROM mikrotik_routers WHERE id=?", id);
    run("UPDATE customers SET router_id=NULL WHERE router_id=?", id);
    if (wasPrimary) {
      const next = get("SELECT * FROM mikrotik_routers WHERE enabled=1 ORDER BY id LIMIT 1");
      if (next) {
        run("UPDATE mikrotik_routers SET is_primary=1 WHERE id=?", next.id);
        Routers.syncSettings(Routers.get(next.id));
      }
    }
...
export const Naps = {
  ...
  remove: (id) => { run("UPDATE customers SET nap_id=NULL WHERE nap_id=?", id); FiberPatches.removeByNap(id); return run("DELETE FROM naps WHERE id=?", id); },
};
...
db.exec(`CREATE TABLE IF NOT EXISTS fiber_patches (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  olt_id INTEGER,
  nap_id INTEGER,
...
ensureColumn("fiber_patches", "path", "TEXT DEFAULT '[]'");
ensureColumn("fiber_patches", "color", "TEXT DEFAULT '#f59e0b'");
ensureColumn("fiber_patches", "route_stops", "TEXT DEFAULT '[]'");
ensureColumn("fiber_patches", "nap_ids", "TEXT DEFAULT '[]'");
(function migrateFiberPatchesNullable() {
  try {
    const row = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='fiber_patches'").get();
    if (row && row.sql && /olt_id\s+INTEGER\s+NOT\s+NULL/i.test(row.sql)) {
      db.exec(`CREATE TABLE fiber_patches_new (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        olt_id INTEGER,
        nap_id INTEGER,
        waypoints TEXT DEFAULT '[]',
      ...
      db.exec(`INSERT INTO fiber_patches_new (id,olt_id,nap_id,waypoints,path,color,notes,created_at)
        SELECT id,olt_id,nap_id,waypoints,'[]','#f59e0b',notes,created_at FROM fiber_patches`);
      db.exec("DROP TABLE fiber_patches");
      db.exec("ALTER TABLE fiber_patches_new RENAME TO fiber_patches");
    }
  } catch {}
})();
...
function parseWaypoints(raw) {
  try {
    ...
    return arr.map((p) => [Number(p[0] ?? p.lat), Number(p[1] ?? p.lng)]).filter((p) => isFinite(p[0]) && isFinite(p[1]));
  } catch { return []; }
}

function normalizePatchColor(c) {
  const s = String(c || "").trim();
  return /^#[0-9A-Fa-f]{6}$/.test(s) ? s.toLowerCase() : "#f59e0b";
}
...
function parseRouteStops(raw) {
  try {
    ...
    if (!Array.isArray(arr)) return [];
    return arr.map((s) => {
      if (s && s.type === "nap" && s.id != null) return { type: "nap", id: Number(s.id) };
      if (s && s.type === "point" && isFinite(+s.lat) && isFinite(+s.lng)) return { type: "point", lat: +s.lat, lng: +s.lng };
      return null;
    }).filter(Boolean);
  } catch { return []; }
}

function parseNapIds(raw) {
  try {
    const arr = JSON.parse(raw || "[]");
    if (!Array.isArray(arr)) return [];
    return arr.map((id) => Number(id)).filter((id) => isFinite(id) && id > 0);
  } catch { return []; }
}

function patchCoordsFromParts(oltLat, oltLng, routeStops, legacyWaypoints, legacyNapLat, legacyNapLng) {
  const pts = [];
  if (oltLat != null && oltLng != null) pts.push([+oltLat, +oltLng]);
  const stops = routeStops && routeStops.length ? routeStops : [];
  if (stops.length) {
    stops.forEach((s) => {
      if (s.type === "point") pts.push([s.lat, s.lng]);
      else if (s.type === "nap") {
        const n = get("SELECT lat,lng FROM naps WHERE id=?", s.id);
        if (n && n.lat != null && n.lng != null) pts.push([+n.lat, +n.lng]);
      }
    });
  } else {
    (legacyWaypoints || []).forEach((w) => { if (w && isFinite(+w[0]) && isFinite(+w[1])) pts.push([+w[0], +w[1]]); });
    if (legacyNapLat != null && legacyNapLng != null) pts.push([+legacyNapLat, +legacyNapLng]);
  }
  return pts;
}

function validatePatchRoute(oltId, path, routeStops) {
  const olt = oltId ? Olts.get(oltId) : null;
  const coords = path.length >= 2 ? path : patchCoordsFromParts(olt?.lat, olt?.lng, routeStops, [], null, null);
  if (coords.length < 2) throw new Error("Draw at least 2 points, or connect OLT to one or more NAPs.");
}

export const Olts = {
  list: () => all("SELECT * FROM olts ORDER BY name COLLATE NOCASE"),
  get: (id) => get("SELECT * FROM olts WHERE id=?", id),
  create: (o) => {
    const r = run("INSERT INTO olts (name,lat,lng,notes) VALUES (?,?,?,?)",
      o.name, o.lat != null && o.lat !== "" ? Number(o.lat) : null, o.lng != null && o.lng !== "" ? Number(o.lng) : null, o.notes || "");
  ...
  update: (id, o) => {
    ...
    run("UPDATE olts SET name=?, lat=?, lng=?, notes=? WHERE id=?",
  ...
  remove: (id) => { FiberPatches.removeByOlt(id); return run("DELETE FROM olts WHERE id=?", id); },
};

export const FiberPatches = {
  list: () => all(`SELECT p.*, o.name AS olt_name, o.lat AS olt_lat, o.lng AS olt_lng,
      n.name AS nap_name, n.lat AS nap_lat, n.lng AS nap_lng
    FROM fiber_patches p
    LEFT JOIN olts o ON o.id = p.olt_id
    LEFT JOIN naps n ON n.id = p.nap_id
    ORDER BY p.id DESC`),
  get: (id) => get(`SELECT p.*, o.name AS olt_name, o.lat AS olt_lat, o.lng AS olt_lng,
      n.name AS nap_name, n.lat AS nap_lat, n.lng AS nap_lng
    FROM fiber_patches p LEFT JOIN olts o ON o.id = p.olt_id LEFT JOIN naps n ON n.id = p.nap_id WHERE p.id=?`, id),
  create: (p) => {
    const path = parseWaypoints(JSON.stringify(p.path || []));
    const wps = parseWaypoints(JSON.stringify(p.waypoints || []));
    const routeStops = parseRouteStops(JSON.stringify(p.route_stops || []));
    const napIds = parseNapIds(JSON.stringify(p.nap_ids || []));
    const oltId = p.olt_id ? Number(p.olt_id) : null;
    let napId = p.nap_id ? Number(p.nap_id) : null;
    if (!napId && napIds.length) napId = napIds[napIds.length - 1];
    const resolvedStops = routeStops.length ? routeStops : [
      ...wps.map((w) => ({ type: "point", lat: w[0], lng: w[1] })),
      ...(napId ? [{ type: "nap", id: napId }] : []),
    ];
    const resolvedNapIds = napIds.length ? napIds : resolvedStops.filter((s) => s.type === "nap").map((s) => s.id);
    validatePatchRoute(oltId, path, resolvedStops);
    if (oltId && !Olts.get(oltId)) throw new Error("OLT not found");
    for (const id of resolvedNapIds) {
      if (!get("SELECT id FROM naps WHERE id=?", id)) throw new Error("NAP #" + id + " not found");
    }
    const color = normalizePatchColor(p.color);
    const r = run("INSERT INTO fiber_patches (olt_id,nap_id,nap_ids,waypoints,route_stops,path,color,notes) VALUES (?,?,?,?,?,?,?,?)",
      oltId, napId || null, JSON.stringify(resolvedNapIds), JSON.stringify(wps), JSON.stringify(resolvedStops), JSON.stringify(path), color, p.notes || "");
    return FiberPatches.get(r.lastInsertRowid);
  },
  update: (id, p) => {
    const cur = get("SELECT * FROM fiber_patches WHERE id=?", id);
    if (!cur) throw new Error("Patch not found");
    const wps = p.waypoints != null ? parseWaypoints(JSON.stringify(p.waypoints)) : parseWaypoints(cur.waypoints);
    const path = p.path != null ? parseWaypoints(JSON.stringify(p.path)) : parseWaypoints(cur.path);
    const routeStops = p.route_stops != null ? parseRouteStops(JSON.stringify(p.route_stops)) : parseRouteStops(cur.route_stops);
    const napIds = p.nap_ids != null ? parseNapIds(JSON.stringify(p.nap_ids)) : parseNapIds(cur.nap_ids);
    const oltId = p.olt_id != null ? (p.olt_id ? Number(p.olt_id) : null) : cur.olt_id;
    let napId = p.nap_id != null ? (p.nap_id ? Number(p.nap_id) : null) : cur.nap_id;
    const resolvedStops = routeStops.length ? routeStops : [
      ...wps.map((w) => ({ type: "point", lat: w[0], lng: w[1] })),
      ...(napId ? [{ type: "nap", id: napId }] : []),
    ];
    const resolvedNapIds = napIds.length ? napIds : resolvedStops.filter((s) => s.type === "nap").map((s) => s.id);
    if (!napId && resolvedNapIds.length) napId = resolvedNapIds[resolvedNapIds.length - 1];
    validatePatchRoute(oltId, path, resolvedStops);
    const color = p.color != null ? normalizePatchColor(p.color) : normalizePatchColor(cur.color);
    run("UPDATE fiber_patches SET olt_id=?, nap_id=?, nap_ids=?, waypoints=?, route_stops=?, path=?, color=?, notes=? WHERE id=?",
      oltId, napId, JSON.stringify(resolvedNapIds), JSON.stringify(wps), JSON.stringify(resolvedStops), JSON.stringify(path), color,
      p.notes != null ? p.notes : cur.notes, id);
    return FiberPatches.get(id);
  },
  remove: (id) => run("DELETE FROM fiber_patches WHERE id=?", id),
  removeByOlt: (oltId) => run("DELETE FROM fiber_patches WHERE olt_id=?", oltId),
  removeByNap: (napId) => {
    const rows = all("SELECT id, nap_id, nap_ids, route_stops FROM fiber_patches");
    rows.forEach((row) => {
      const ids = parseNapIds(row.nap_ids);
      const stops = parseRouteStops(row.route_stops);
      const stopIds = stops.filter((s) => s.type === "nap").map((s) => s.id);
      if (Number(row.nap_id) === Number(napId) || ids.includes(Number(napId)) || stopIds.includes(Number(napId))) {
        run("DELETE FROM fiber_patches WHERE id=?", row.id);
      }
    });
  },
  public: (p) => {
    if (!p) return null;
    const route_stops = parseRouteStops(p.route_stops);
    const nap_ids = parseNapIds(p.nap_ids);
    const nap_names = nap_ids.map((nid) => get("SELECT name FROM naps WHERE id=?", nid)?.name).filter(Boolean);
    return {
      ...p,
      waypoints: parseWaypoints(p.waypoints),
      path: parseWaypoints(p.path),
      route_stops,
      nap_ids,
      nap_names,
      color: normalizePatchColor(p.color),
    };
  },
};
